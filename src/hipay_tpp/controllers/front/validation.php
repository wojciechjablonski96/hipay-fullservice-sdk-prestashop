...
* @license   https://github.com/hipay/hipay-fullservice-sdk-prestashop/blob/master/LICENSE.md
*
*/
...
class HiPay_TppValidationModuleFrontController extends ModuleFrontController {
	/**
	 *
	...
	public function postProcess() {
		...
		$this->HipayLog('##########################################');
		$this->HipayLog('##########################################');
		$this->HipayLog('DEBUT TRAITEMENT DU CALLBACK');
		$this->HipayLog(print_r($callback_arr,TRUE));
		...
		//LOG 
		$this->HipayLog('---------- $log_state = '.$log_state);
		$this->HipayLog('---------- $num_status = '.$num_status);
		$this->HipayLog('---------- $log_status = '.$log_status);
		...
		//LOG 
		$this->HipayLog('---------- $callback id cart = '.$callback_arr['order']['id']);
		...
		//LOG
		$this->HipayLog('---------- Boutique N°'.$cart->id_shop);
		//LOG 
		$this->HipayLog('---------- panier exist ID = '.$cart->id);
		...
		try{
			...
			// LOG
			$this->HipayLog('Vérification configuration statut = [ '.print_r($arrStatut,true).' ]');

			//LOG 
			$this->HipayLog('---------- config statut intialisé');
			...
			//LOG 
			$this->HipayLog('---------- début ctrl order existe');
			...
			//LOG 
			$this->HipayLog('---------- order_exist = '. $order_exist);
			$this->HipayLog('---------- id_order = '. $id_order);
			if($id_order){
				$this->HipayLog('---------- objOrder initialisé');
			}
			...
			switch ($num_status) {
				...
				case 129 : // Charged Back
					...
					$this->changeStatusOrder($order_exist, $id_order, $orderState, $objOrder);
					// si pas de commande existante, on créé la commande
					$this->createOrderByHipay($order_exist,$callback_arr, $hipay, $cart, $orderState);
					break;
				...
				case HIPAY_STATUS_REFUSED : // Refused
					...
					if(!$this->controleIfStatushistoryExist($id_order, $stt_payment, $orderState, true)){
						$this->changeStatusOrder($order_exist, $id_order, $orderState, $objOrder);
						// si pas de commande existante, on créé la commande
						$this->createOrderByHipay($order_exist,$callback_arr, $hipay, $cart, $orderState);
					}
				...
				case HIPAY_STATUS_AUTHORIZED :
					...
					$this->changeStatusOrder($order_exist, $id_order, $orderState, $objOrder);
					// si pas de commande existante, on créé la commande
					$this->createOrderByHipay($order_exist,$callback_arr, $hipay, $cart, $orderState, $objOrder);
					break;
				...
				case HIPAY_STATUS_CAPTURE_REQUESTED : // Capture Requested
					...
					if($this->changeStatusOrder($order_exist, $id_order, $orderState, $objOrder, $callback_arr)){
						// on modifie la transaction de commande pour la référence commande
						$this->captureOrder($callback_arr, $objOrder);
					}
					// si pas de commande existante, on créé la commande
					$this->createOrderByHipay($order_exist,$callback_arr, $hipay, $cart, $orderState);
					break;
				...
				case 119 : // Partially Captured
					$orderState = $stt_partially;
					// on change de statut de la commande tous les critères sont ok
					if($this->changeStatusOrder($order_exist, $id_order, $orderState, $objOrder)){
						// on modifie la transaction de commande pour la référence commande
						$this->captureOrder($callback_arr, $objOrder);
					}
					// si pas de commande existante, on créé la commande
			...
			//LOG 12
			$this->HipayLog('---------- Fin de traitement');
			...
			//LOG 13
			$this->HipayLog('********** Fin LOCK = ' .$sql);
			$this->HipayLog('FIN TRAITEMENT DU CALLBACK');
			$this->HipayLog('##########################################');
			$this->HipayLog('##########################################');
	 ...
	 */
	protected function addMessageRefund($order, $callback_arr, $hipay, $statuts = NULL){
		//LOG 
		$this->HipayLog('--------------- Début $this->addMessageRefund');

		if($statuts !== NULL){
				
			//LOG 
			$this->HipayLog('--------------- $this->addMessageRefund - init message');
			...
			if (Validate::isCleanHtml($message)) {
				...
				//LOG 
				$this->HipayLog('--------------- $this->addMessageRefund - fin');
	 ...
	 */
	protected function createOrderByHipay($order_exist,$callback_arr, $hipay, $cart, $statut, $order = false){
		$bool = false;
		//LOG 
		$this->HipayLog('--------------- Début $this->createOrderByHipay');
		if(!$order_exist){
			//LOG
			$this->HipayLog('--------------- pas de commande existante');
			...
			$this->HipayLog('--------------- Start validateOrder ');
			//LOG
			$this->HipayLog('--------------- SHOP Id = '.$cart->id_shop);
			$this->HipayLog('--------------- shop obj' . print_r($shop, true));
			try{
				$hipay->validateOrder(
				...
				$this->HipayLog($e->getCode().' : ' . $e->getMessage());
			}
			$this->HipayLog('--------------- Fin validateOrder ');
			// init order for message HIPAY_CAPTURE
			$id_order = $hipay->currentOrder;
			// LOG
			$this->HipayLog('--------------- ID Current Order = '.$id_order);
			// Init / MAJ de la ligne message HIPAY_CAPTURE
			$this->addHipayCaptureMessage($callback_arr,$id_order);
			$new_order = new order($id_order);
			$this->HipayLog('--------------- ID Current status = '.$new_order->current_state);
			
			// MAJ ligne transaction pour le status 116
			if ($callback_arr['status'] == HIPAY_STATUS_AUTHORIZED) {	
				// LOG
				$this->HipayLog('--------------- STATUS 116 début update order payment');		
				...
				// LOG
				$this->HipayLog('--------------- STATUS 116 fin update order payment');
			}
			...
			// LOG
	        $this->HipayLog('--------------- TABLE HIPAY = '. $sql);
	        if(!Db::getInstance()->execute($sql)){
	        	//LOG 
				$this->HipayLog('Insert table HiPay en erreur');
				return false;
	        }
			...
			if ($callback_arr['payment_product'] == 'american-express' || $callback_arr['payment_product'] == 'cb' || $callback_arr['payment_product'] == 'visa' || $callback_arr['payment_product'] == 'mastercard') {
				...
				if (!$result['id']) {
					// LOG
	        		$this->HipayLog('--------------- Enregistrement nouveau Token');
					// 'insert in DB';
			...
			//LOG
			$this->HipayLog('--------------- currentOrder = '.$hipay->currentOrder);
			$bool = true;
			...
			if ($order && Validate::isLoadedObject($order)) {
				...
				if ($order->addOrderPayment($amount, $payment_method, $payment_transaction_id, $currency, $payment_date, $order_invoice)) {
					// LOG
					$this->HipayLog('--------------- Order payment add with success');
	 ...
	 */
	protected function changeStatusOrder($order_exist, $id_order, $orderState, $order, $callback_arr = ''){
		$bool = false;
		//LOG 
		$this->HipayLog('--------------- Début $this->changeStatusOrder');

		if($order_exist && $id_order){
			//LOG 
			$this->HipayLog('--------------- oderexist && id_order');
			if ((int)$order->getCurrentState() != (int)$orderState && !$this->controleIfStatushistoryExist($id_order, _PS_OS_PAYMENT_, $orderState)){
				//LOG 
				$this->HipayLog('--------------- statut différent');
				$order_history = new OrderHistory();
				//LOG 
				$this->HipayLog('--------------- order_history init');
				$order_history->id_order = $id_order;
				//LOG 
				$this->HipayLog('--------------- order_id init');
				$order_history->changeIdOrderState($orderState, $id_order,true);
				//LOG 
				$this->HipayLog('--------------- changeIdOrderState('.$orderState.','.$id_order.')');
				$order_history->add();

				//LOG 
				$this->HipayLog('--------------- statut changé = '.$orderState);
			...
			// Init / MAJ de la ligne message HIPAY_CAPTURE
			$this->addHipayCaptureMessage($callback_arr,$order->id);

			//LOG
			$this->HipayLog('--------------- statut est le même');

			// Controle if it's again a partially captured and inferior to the total order

			//LOG 
			$this->HipayLog('--------------- Controle si status 117 et amount > amount captured');
			...
			if(!empty($callback_arr) 
				...
				//LOG 
				$this->HipayLog('--------------- captured_amount ('.$callback_arr['captured_amount'].') est < à authorized_amount ('.$callback_arr['authorized_amount'].')');
				$bool = true;
			}
		...
		//LOG 
		$this->HipayLog('--------------- pas de changement de statut car pas de commande');
		return $bool;
	}
	/*
	 ...
	 */
	protected function captureOrder($callback_arr = null, $order = null) {

		//LOG 
		$this->HipayLog('--------------- Début $this->captureOrder');
		$hipay = new HiPay_Tpp();
		// Local Cards update
		...
		if (isset($callback_arr['payment_method']['token'])) {
	        ...
	        if(!Db::getInstance()->execute($sql)){
	        	//LOG 
				$this->HipayLog('--------------- Update en erreur');
				return false;
	        }
			...
			//LOG 
			$this->HipayLog('--------------- delete transaction pas invoice = '. $sql_duplicate_order_payment);
			...
			if (Validate::isCleanHtml($message)) {
				...
				//LOG 
				$this->HipayLog('--------------- addMessage on $this->captureOrder - ' . $message);
			}
	    }
	    if($callback_arr)
		//LOG 
		$this->HipayLog('--------------- Fin $this->captureOrder');
		$this->HipayLog('--------------- --------------- --------------- --------------- ');
		$this->HipayLog('--------------- --------------- --------------- --------------- ');
	 ...
	 */
	protected function refundOrder($callback_arr, $order, $hipay, $statut) {
		
		//LOG 
		$this->HipayLog('--------------- Début $this->refundOrder'); 
		...
		$this->HipayLog('--------------- SQL Si déjà un refund = '. $payment_message_sql);

		$paymentmessage = Db::getInstance()->executeS($payment_message_sql);
		if (!empty($paymentmessage)) {

			// LOG
			$this->HipayLog('--------------- Refund existant donc procédure update');

			// Set refund to negative
			$amount = - 1 * $callback_arr['refunded_amount'];

			// LOG
			$this->HipayLog('--------------- Montant à refund = '.$amount);
			...
			// LOG
			$this->HipayLog('--------------- Nouveau Refund donc procédure insert');
		...
		//LOG
		$this->HipayLog('--------------- SQL Update Order = ' . $sql_status); 

		if(!Db::getInstance()->execute($sql_status)){
			//LOG 
			$this->HipayLog('--------------- Update statut = '.$statut.' en erreur pour la commande = '.$order->id);
			return false;
		} 
		//LOG
		$this->HipayLog('--------------- SQL Update Order OK'); 	
		$this->HipayLog('--------------- fin $this->refundOrder'); 
	 ...
	 */
	protected function addHipayCaptureMessage($callback_arr,$id_order){
		//LOG
		$this->HipayLog('--------------- Début addHipayCaptureMessage');
		// Init / MAJ de la ligne message HIPAY_CAPTURE
		$tag = 'HIPAY_CAPTURE ';
		...
		//LOG
		$this->HipayLog('--------------- Fin addHipayCaptureMessage');
	}
	/*
	 ...
	 */
	protected function controleIfStatushistoryExist($id_order, $stt_payment, $orderState, $forceCtrl = false){
		//LOG
		$this->HipayLog('--------------- Début contrôle si déjà eu paiement accepté');
		...
		if( $orderState == $stt_payment || $forceCtrl ) {
			$bGoGo = true;
			//LOG
			$this->HipayLog('--------------- Control action = TRUE');
		}
		...
		if($bGoGo && $id_order){
			...
			//LOG
			$this->HipayLog('--------------- SQL de contrôle = ' . $sql);
	    ...
	    //LOG
	    $this->HipayLog('--------------- Statut traité => ' . $orderState);
	    $this->HipayLog('--------------- Résultat - Existe d�a ? => ' . ($bool ? 'TRUE':'FALSE'));
		$this->HipayLog('--------------- Fin contrôle si dja eu paiement accepté');
	...
	#
	protected function HipayLog($msg){
		if(HIPAY_LOG){
