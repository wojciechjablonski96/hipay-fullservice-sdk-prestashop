...
class HipayMaintenance extends ObjectModel {
	 ...
	 */
	public static function restMaintenanceApi($transaction_reference = null, $data = null, $shop_id=null) {
		try {
			$hipay = new HiPay_Tpp ();
			...
			define ( 'API_USERNAME', HipayClass::getAPIUsername ($shop_id) );
			define ( 'API_PASSWORD', HipayClass::getAPIPassword ($shop_id) );
			...
			HipayLogger::addLog ( $hipay->l ( 'API call success', 'hipay' ), HipayLogger::APICALL, 'Appel vers API avec success : ' . Db::getInstance()->escape ( Tools::jsonEncode($response) ) );
			return $response;
		} catch ( Exception $e ) {
